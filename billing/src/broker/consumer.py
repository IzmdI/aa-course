...
from billing.src.broker.dependencies import broker_settings, get_user_broker_service, get_billing_broker_service
...
async def consume_tasks(broker_settings: Broker_settings) -> None:
    ...
    billing_service: BrokerBillingService = await get_billing_broker_service()
    await consumer.start()
    try:
        async for msg in consumer:
            match msg.topic:
                ...
                case _:
                    continue
            if validator.is_valid(msg.value):
                await billing_service(event(**msg.value).event_data)
            else:
                # TODO: невалидные сообщения будем сваливать в мёртвую очередь,
                #  которую будет разбирать специально обученный попуг
                continue
